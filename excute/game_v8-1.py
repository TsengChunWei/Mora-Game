    ...
    def currResult(self):
        ...
        if winner == 'equal':
            ...
            while count <= 0.8: 
                count = time.time() - start_time
                if 0.5 < count:
                    temp = self.animng.star_appear(count, pre_frame, self.imgShow.star_mask, self.imgShow.scorePosition("COM", self.comScore-1))
                outputFrame = self.animng.character_shake(count, temp, self.character.win1, self.character.win2)
                cv2.imshow('frame', outputFrame)
                if cv2.waitKey(1) == 27:
                    self.endWindows = True
                    return
            ...
            while count <= 0.8: 
                count = time.time() - start_time
                if 0.5 < count:
                    temp = self.animng.star_appear(count, pre_frame, self.imgShow.star_mask, self.imgShow.scorePosition("player", self.playerScore-1))
                outputFrame = self.animng.character_shake(count, temp, self.character.lose1, self.character.lose2)
                cv2.imshow('frame', outputFrame)
                if cv2.waitKey(1) == 27:
                    self.endWindows = True
                    return
