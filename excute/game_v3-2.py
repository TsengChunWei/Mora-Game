...
class ScoreShow():
    ...
    def scoreshow(self, outputFrame, p, score):
        for i in range(score):
            overlay_img = self.imgInsert.extend_alpha(self.star_mask, self.position(p, i), self.window.shape)
            outputFrame = self.imgInsert.mask_operate(outputFrame, overlay_img)
        return outputFrame
    
    def position(self, p, score):
        if p == "COM":
        ...
        while currRound <= self.initVal.tolRound:
            ...
            while counting > 0:
                ...
                if ret:
                    ...
                    outputFrame = scoreshow.scoreshow(outputFrame, "COM", comScore)
                    outputFrame = scoreshow.scoreshow(outputFrame, "player", playerScore)
            ...
            if counting <= 0:
                ...
                if winner == "equal":
                    ...
                    while count <= 0.8: 
                        count = time.time() - start_time
                        scoreshow.comWinAnime(round(100*count), outputFrame, temp, scoreshow.position("COM", comScore-1))
                        if cv2.waitKey(1) == 27:
                            endWindows = True
                    ...
                    while count <= 0.8: 
                        count = time.time() - start_time
                        scoreshow.comWinAnime(round(100*count), outputFrame, temp, scoreshow.position("player", playerScore-1))
                        if cv2.waitKey(1) == 27:
                            endWindows = True
                    self.Timer(self.initVal.break_time-0.8)
...
moragame = MoraGame(f'{PF.folder}/model_pt/1005_x478_4.pt')
