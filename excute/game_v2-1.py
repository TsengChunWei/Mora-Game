...
class ComMoraImgInsert():
    def __init__(self) -> None:
        ...
        self.new_w, self.new_h = 300, 300
        self.rock_img = cv2.imread(f'{os.path.dirname(__file__)}/../images/rock_alpha2.png')
        self.paper_img = cv2.imread(f'{os.path.dirname(__file__)}/../images/paper_alpha2.png')
        self.scissors_img = cv2.imread(f'{os.path.dirname(__file__)}/../images/scissors_alpha2.png')
    ...
    def img_insert(self, frame, commora):
        insert_img = self.commoraimg(commora)
        resized_image = cv2.resize(insert_img, (self.new_w, self.new_h))
        frame[self.y:self.y+self.new_h, self.x:self.x+self.new_w] = resized_image
        return frame
...
class MoraGame():
    def __init__(self) -> None:
        ...
        self.w_min = (self.winSize.width*300)//640
        self.w_max = (self.winSize.width*550)//640
        self.h_min = (self.winSize.height*200)//480
        self.h_max = (self.winSize.height*460)//480
    ...
    def main(self):
        ...
        model = YOLO(f"{os.path.dirname(__file__)}/../model_pt/1002.pt")

        # cap = cv2.VideoCapture(f"{os.path.dirname(__file__)}/../test_video/test_video.mp4")
        ...
        while rounds <= Round:
            ...
            if counting == 0:
                ...
                annotated_frame = commoraimginsert.img_insert(annotated_frame, comMora)
