...
class ComCharacter():
    def __init__(self, person) -> None:
        ...
        self.win1 = cv2.imread(f'{PF.folder}/images/character/{person}/action/win_act_1.png', -1)
        self.win2 = cv2.imread(f'{PF.folder}/images/character/{person}/action/win_act_2.png', -1)
        self.lose1 = cv2.imread(f'{PF.folder}/images/character/{person}/action/lose_act_1.png', -1)
        self.lose2 = cv2.imread(f'{PF.folder}/images/character/{person}/action/lose_act_2.png', -1)
        self.tie1 = cv2.imread(f'{PF.folder}/images/character/{person}/action/tie_act_1.png', -1)
        self.tie2 = cv2.imread(f'{PF.folder}/images/character/{person}/action/tie_act_2.png', -1)
...
class AnimeManage():
    ...
    def star_appear(self, count, inputFrame, star_mask, point):
        resize_mask = cv2.resize(star_mask, (176-round(100*count), 176-round(100*count)))
...
class Opening():
    ...
    def animation(self, cap):
        ...
        t = time.time()
        while time.time()-t < 1:
            cv2.imshow('frame', self.boxer_hit3)
            if cv2.waitKey(1) == 27:
                return
