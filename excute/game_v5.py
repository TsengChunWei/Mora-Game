...
class MoraGame():
    ...
    def GamingPlay(self, video_file = None):
        ...
        endWindows = False
        counting = self.initVal.timeCount
        currRound, playerScore, comScore = 1, 0, 0
        ...
        while currRound <= self.initVal.tolRound:
            ...
            while counting > 0:
                ...
                if ret:
                    ...
                    counting = self.initVal.timeCount-round(time.time()-begin)
                    ...
                    overlay_img = extend_alpha(character.normal, (0, self.window.shape[1]-512), self.window.shape)
            ...
            if counting == 0:
                ...
                overlay_img = extend_alpha(commoraimg.imgresult(comMora), (self.window.shape[0]//2-256, self.window.shape[1]//2-256), (self.window.shape[0], self.window.shape[1]))
                outputFrame = mask_operate(tempFrame, overlay_img)

                winner = self.who_win(self.playerMora, comMora)

                if winner == 'equal':
                    currRound -= 1
                    overlay_img = extend_alpha(character.tie, (0, self.window.shape[1]-512), self.window.shape)
                    outputFrame = mask_operate(outputFrame, overlay_img)
                elif winner == 'COM':
                    comScore += 1
                    overlay_img = extend_alpha(character.win, (0, self.window.shape[1]-512), self.window.shape)
                    outputFrame = mask_operate(outputFrame, overlay_img)
                else:
                    playerScore += 1
                    overlay_img = extend_alpha(character.lose, (0, self.window.shape[1]-512), self.window.shape)
                    outputFrame = mask_operate(outputFrame, overlay_img)
                ...
                counting = self.initVal.timeCount
                if comScore < self.initVal.winCondition and playerScore < self.initVal.winCondition:
                    currRound += 1
                    continue

                ED = ending()
                ED.ed_show(comScore, self.initVal.winCondition, 3)
                endWindows = True
                break
